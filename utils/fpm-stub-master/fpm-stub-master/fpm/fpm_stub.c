...
void consumeRest(int i){
  ...
  curl = curl_easy_init();
  if(curl) {
    curl_easy_setopt(curl, CURLOPT_URL, "http://127.0.0.1:5000/snmp/atp/192.168.1.13");
...
void *timer(void *arguments){
	...
	for(;;){
		...
		fprintf(stderr, "processTopo = : %d, time= %d\n", args->processTopology, args->time);
		if (args->processTopology)
			args->time++;
			if (args->time >= 5){
				args->time =0;	
				consumeRest(1);				
				args->processTopology =0;			
			}
	}
...
fpm_serve ()
{
  ...
  int processTopology = 0;
  //CREO EL THREAD
  pthread_t  thread;
  printf("CREO FPM_SERVE");
  struct arg_struct *args = malloc(sizeof(struct arg_struct));
  if(pthread_create(&thread,NULL,timer,(void*)args ))	;
 
  char buf[FPM_MAX_MSG_LEN];
  fpm_msg_hdr_t *hdr;

  while (1) {
	fprintf(stderr, "Voy a escuchar\n");
    hdr = read_fpm_msg(buf, sizeof(buf));
    args->time = 0;
    args->processTopology = 1;
    fprintf(stderr, "Llego paquete\n");	
    if (!hdr) {
      return;
    }
