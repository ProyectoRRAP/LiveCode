...
var divWrapper = d3.select("#tab-1-topo");
var elem = {
    force: d3.layout.force()
        .size([CONF.force.width, CONF.force.height])
        .charge(CONF.force.charge)
...
elem.update = function () {
    ...
    var linkEnter = this.link.enter().append("line").attr("class", "link");
    /*linkEnter.attr("stroke-width", "10px");*/
    linkEnter.attr("id", function(d) {return parseInt(d.port.src.dpid, 16) + '-' + trim_zero(d.port.src.port_no) + '-' + parseInt(d.port.dst.dpid, 16) + '-' + trim_zero(d.port.dst.port_no) });
    ...
    var nodeEnter = this.node.enter().append("g")
        ...
        .call(this.drag);
    nodeEnter.attr("id", function(d) { return parseInt(d.dpid, 16); });
...
var rpc = {
    ...
    /* Ours events */
    event_lsdb_link_delete: function (links) {
	console.log('Link LSDB leave');
	console.log(links);
        topo.delete_links(links);
        elem.update();
        return "";
    },
    event_lsdb_switch_leave: function (params) {
        console.log('Switch LSDB leave');
	console.log(params);
